...
// Max N for convenience, typically N+1 for 1-based indexing
const int MAXN = 501;
...
int main() {
    ...
    // Initialize dist array with -1 (unvisited)
    // Using memset for efficiency on large arrays
    memset(dist, -1, sizeof(dist));

    // BFS queue stores pairs of (Bob's position, Alex's position)
    ...
    while (!q.empty()) {
        ...
        // Iterate through all possible next moves for Bob
        for (int bob_next : adj[bob_curr]) {
            // Iterate through all possible next moves for Alex
            for (int alex_next : adj[alex_curr]) {
                // Collision check: they must not land on the same crossroads at the same time
                if (bob_next == alex_next) {
                    continue; // Collision, skip this pair of moves
                }
    ...
    if (final_k == -1) {
        ...
        cout << final_k << endl; // Print the minimum length of routes

        vector<int> bob_path;
        vector<int> alex_path;

        int curr_bob_pos = n; // Start reconstruction from Bob's destination
        int curr_alex_pos = 1; // Start reconstruction from Alex's destination
